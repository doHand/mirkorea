  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">창고 위치 관리 — {warehouse.name}</h2>
        <button
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h3 className="font-medium text-gray-900 mb-3">구역</h3>
          <div className="space-y-1">
            <button
              onClick={() => setSelectedZone('')}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                !selectedZone ? 'bg-brand-100 text-brand-700 font-medium' : 'hover:bg-gray-50 text-gray-700'
              }`}
            >
              전체
            </button>
            {zones.map((z: any) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                  selectedZone === z.id ? 'bg-brand-100 text-brand-700 font-medium' : 'hover:bg-gray-50 text-gray-700'
                }`}
              >
                {z.code} — {z.name}
              </button>
        ...
        <div className="col-span-2 bg-white rounded-xl border border-gray-200">
          <div className="p-4 border-b border-gray-100">
            <h3 className="font-medium text-gray-900">위치 목록 ({locations.length})</h3>
          </div>
          <div className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
            {locations.map((loc: any) => (
              <div key={loc.id} className="flex items-center p-3 gap-3">
                <MapPin size={14} className="text-gray-400 shrink-0" />
                <div className="flex-1">
                  <p className="text-sm font-mono font-medium">{loc.code}</p>
                  <p className="text-xs text-gray-400">
                    {[loc.aisle && `통로:${loc.aisle}`, loc.rack && `랙:${loc.rack}`, loc.shelf && `단:${loc.shelf}`]
                      .filter(Boolean).join(' · ')}
                  </p>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full ${loc.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
      ...
      {showLocationModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-4">위치 추가</h3>
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">위치코드 *</label>
                <input
                  value={locForm.code}
