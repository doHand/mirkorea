...
function printQuoteDocument(doc: Quote, targetWindow?: Window | null, client?: Client | null, supplier: SupplierInfo = DEFAULT_SUPPLIER_INFO) {
  const isStatement = doc.docType === 'STATEMENT'
  const title = isStatement ? '거 래 명 세 서' : '견   적   서'
  ...
  const rows = Array.from({ length: lineCount }, (_, idx) => {
    ...
    if (!item) return `<tr><td>${idx + 1}</td><td></td><td></td><td></td><td></td><td></td></tr>`
    return `<tr>
      <td>${idx + 1}</td>
  ...
  const quoteHeader = `
    ...
    <div class="quote-to">
      <div>서기 : ${escapeHtml(doc.docDate)}</div>
    ...
    <div class="quote-total-line"><span>합계금액 : ${money(total)} 원정</span><span>( ${money(total)} )</span><span>부가세별도</span></div>`
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .quote { color: #000; --line: #000; border: 1px solid var(--line); padding: 8mm 0 5mm; }
    ...
    .stamp { position: absolute; left: calc(50% - 9mm); top: 8mm; color: #e5002b; border: 2px solid #e5002b; width: 16mm; height: 16mm; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; transform: rotate(-12deg); z-index: 1; background: transparent; }
    .quote-to { position: absolute; left: 2mm; top: 34mm; width: 88mm; line-height: 2.1; }
    .quote-to .client-line { margin: 4mm 0; border-bottom: 1px solid #000; text-align: center; font-size: 13px; }
    .quote-party { position: relative; display: grid; grid-template-columns: 8mm 1fr; width: 92mm; margin-left: auto; margin-bottom: 0; }
    .quote-total-line { display: grid; grid-template-columns: 1fr 35mm 35mm; align-items: center; height: 9mm; border-left: 1px solid var(--line); border-right: 1px solid var(--line); border-bottom: 1px solid var(--line); font-size: 13px; font-weight: 700; padding: 0 2mm; }
    .items { margin-top: 0; }
    ...
    .footer-page { text-align: right; padding: 4mm 2mm 0 0; font-size: 10px; }
...
<body>
  <div class="sheet ${isStatement ? 'statement' : 'quote'}">
    ...
    <table class="items"><colgroup><col style="width:8mm"><col style="width:87mm"><col style="width:14mm"><col style="width:20mm"><col style="width:26mm"><col style="width:35mm"></colgroup><thead><tr><th>순번</th><th>품명 / 규격</th><th>단위</th><th>수량</th><th>단가</th><th>금액</th></tr></thead><tbody>${rows}</tbody></table>
...
export default function QuotesPage() {
  ...
  const submitForm = async (printAfterSave = false) => {
    ...
    try {
      ...
      if (printAfterSave) printQuoteDocument(saved, printWindow, findClientForQuote(saved), supplierInfo)
    } catch {
      printWindow?.close()
      toast.error('저장 실패')
    }
